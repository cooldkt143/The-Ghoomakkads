    ...
    <div className={`fixed bottom-5 right-5 z-50 ${showChatbot ? '' : ''}`}>
      <span
        ...
        className={`fixed z-50 flex items-center justify-center text-xl font-bold shadow-lg transition-all duration-300 ease-in-out
          ${showChatbot
            ? 'bottom-7 hidden lg:flex right-7 w-[60px]  h-[60px] rounded-full bg-rose-700 text-white hover:bottom-8 hover:right-8 hover:w-[65px] hover:h-[65px] hover:rounded-full'
      ...
      {showChatbot && (
        <div className="fixed bottom-0 lg:bottom-24 right-0 lg:right-5 w-full h-full lg:h-[600px] lg:w-[420px] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
          <div className="flex items-center justify-between bg-rose-700 px-6 py-4 text-white">
            ...
            <span 
            onClick={() => setShowChatbot(false)} className="material-symbols-rounded cursor-pointer bg-rose-700 hover:bg-rose-600 rounded-full sm:rounded-full w-[30px] h-[30px] sm:w-[30px] sm:h-[30px] text-white text-2xl justify-center items-center flex">
              keyboard_arrow_down
            </span>
          </div>

          <div ref={chatBodyRef} className="flex flex-col gap-3 p-6 lg:h-[460px] h-screen overflow-y-auto">
